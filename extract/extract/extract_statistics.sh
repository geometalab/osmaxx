...
gather_statistics(){
	...
	LABEL=$4	#Label to be attached to the stat count in the FILE '$4' is the variable to be used
	...
	2) 
		...
		KEY=3;;
	3)
		TEXT="where type='"$LABEL"' and status='"$val"'"
		LABEL=$LABEL","
		KEY=2;;
	4)
		TEXT="where aggtype='"$LABEL"' and type='"$val"'"
		LABEL=$LABEL","
		KEY=3;;
	esac

	# STARTING COMPLILATION OF STATS
