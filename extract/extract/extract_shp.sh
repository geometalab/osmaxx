...
rm $DIR/data/$FILENAME
	...
	mkdir -p $DIR/data
	ogr2ogr -f "ESRI Shapefile" $DIR/data/$FILENAME PG:"dbname='"$DBNAME"' user='"$USER"' password='"$PASS"' port="$PORT" schemas=view_osmaxx" -clipsrc $XMIN $YMIN $XMAX $YMAX
	echo $FILENAME" have been Generated.. Zipping files"

	cd $DIR
	zip -r --move $DIR/data/$FILENAME.zip ./data/$FILENAME
