...
class EstimatedFileSizeViewTests(TestCase):
    def setUp(self):
        ...
        self.boundaries = {bound: bound + '_value' for bound in ['west', 'south', 'east', 'north']}

        factory = APIRequestFactory()
        self.request = factory.get('/estimated_file_size/', self.boundaries)

    def test_estimated_file_size_view_calls_api_client_with_boundaries(self):
        api_views.estimated_file_size(self.request)
        self.estimated_file_size_api_client_mock.assert_called_with(**self.boundaries)
