...
def reload_until_condition(selenium):
    def _reload_until_condition(
        condition, *args, timeout=360, refresh_interval=5, **kwargs
    ):
        while timeout:
            selenium.refresh()
            try:
...
first_panel_on_excerpts_export_overview_xpath = (
    "/html/body/div/div/div[2]/div[2]/div[1]/div/"
)
