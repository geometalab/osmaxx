...
def test_new_registration_with_email_moves_email_address_correctly(
...
@pytest.mark.django_db()
def test_new_registration_without_email_prompts_the_user_to_add_one(
    authenticated_client,
):
    user = authenticated_client.user
    user.email = ""
...
@pytest.mark.django_db()
def test_registration_with_email_doesnt_move_email_when_already_authorized(
    authorized_client,
):
...
def test_email_change(authorized_client):
    ...
    response = authorized_client.post(
        reverse("profile:edit_view"), {"unverified_email": new_email}, follow=True
    )
