...
class CallbackHandlingTest(APITestCase):
    ...
    def test_calling_tracker_with_nonexistant_export_raises_404_not_found(self):
        ...
        request = factory.get(
            reverse(
                "job_progress:tracker",
                kwargs=dict(export_id=self.nonexistant_export_id),
            ),
            data=dict(
                status="http://localhost:8901/api/conversion_result/53880847-faa9-43eb-ae84-dd92f3803a28/"
            ),
        )

        self.assertRaises(
            Http404, views.tracker, request, export_id=str(self.nonexistant_export_id)
        )
        request.resolver_match

    def test_calling_tracker_with_payload_indicating_queued_updates_export_status(
        self, *args
    ):
        factory = APIRequestFactory()
        request = factory.get(
            ...
            data=dict(
                status="queued", job="http://localhost:8901/api/conversion_job/1/"
            ),
        )
    ...
    def test_calling_tracker_with_payload_indicating_started_updates_export_status(
        ...
        request = factory.get(
            reverse("job_progress:tracker", kwargs=dict(export_id=self.export.id)),
            data=dict(
                status="started", job="http://localhost:8901/api/conversion_job/1/"
            ),
        )
    ...
    def test_calling_tracker_with_payload_indicating_failed_informs_user_with_error(
        ...
        request = factory.get(
            reverse("job_progress:tracker", kwargs=dict(export_id=self.export.id)),
            data=dict(
                status="failed", job="http://localhost:8901/api/conversion_job/1/"
            ),
        )
    ...
    def test_calling_tracker_with_payload_indicating_finished_informs_user_with_success(
        ...
        request = factory.get(
            reverse("job_progress:tracker", kwargs=dict(export_id=self.export.id)),
            data=dict(
                status="finished", job="http://localhost:8901/api/conversion_job/1/"
            ),
        )
    ...
    def test_calling_tracker_with_payload_indicating_status_finished_downloads_result(
        ...
        requests_mock.get(
            ...
            json=dict(resulting_file_path=resulting_file.name),
        )
        requests_mock.get(
...
class ExportUpdaterMiddlewareTest(TestCase):
    ...
    def test_request_middleware_updates_exports_of_request_user(
        self, update_exports_mock
    ):
    ...
    def test_request_middleware_passes_request_with_user(self, update_exports_mock):
        ...
        self.assertIsNotNone(
            request.build_absolute_uri.__call__, msg="not a usable request"
        )
        self.assertIsNotNone(request.user)

    @patch("osmaxx.job_progress.middleware.update_exports_of_request_user")
    def test_request_middleware_when_authenticated_passes_request_with_logged_in_user(
        self, update_exports_mock
    ):
...
class ExportUpdateTest(TestCase):
    def setUp(self):
        ...
        self.own_unfinished_exports = [
            own_order.exports.create(file_format="fgdb", conversion_service_job_id=1)
            for i in range(2)
        ]
        for i in range(4):
            own_order.exports.create(file_format="fgdb")
        for i in range(8):
            own_order.exports.create(
                file_format="fgdb", conversion_service_job_id=1, status=status.FINISHED
            )
        for i in range(16):
            own_order.exports.create(
                file_format="fgdb", conversion_service_job_id=1, status=status.FAILED
            )
        for i in range(32):
            foreign_order.exports.create(
                file_format="fgdb", conversion_service_job_id=1
            )
        self.client.login(username="user", password="pw")

    @patch("osmaxx.job_progress.middleware.update_export_if_stale")
    def test_update_exports_of_request_user_updates_each_unfinished_export_of_request_user(
    ...
    def test_update_exports_of_request_user_does_not_update_any_exports_of_other_users_nor_own_exports_in_a_final_state(
        ...
        self.client.get("/dummy/")
        self.assertEqual(
            update_progress_mock.call_count, len(self.own_unfinished_exports)
        )
    ...
    def test_update_export_if_stale_when_stale_updates_export(self, update_export_mock):
        assert (
            len(self.own_unfinished_exports) > 1
        ...
        update_export_mock.assert_has_calls(
            [
                call(export, request=sentinel.REQUEST)
                for export in self.own_unfinished_exports
            ],
        )
    ...
    @override_settings(CACHES=_LOC_MEM_CACHE)
    @patch("osmaxx.job_progress.middleware.update_export", return_value="updated")
    def test_update_export_if_stale_when_not_stale_does_not_update_export(
        self, update_export_mock
    ):
        from django.core.cache import cache
