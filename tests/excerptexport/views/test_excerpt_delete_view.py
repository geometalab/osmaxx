...
def test_delete_fails_when_exports_in_progress(
    authorized_client, user, extraction_order, bounding_geometry
):
    ...
    Export.objects.create(
        ...
        conversion_service_job_id=None,
        status=None,
        finished_at=None,
    )
...
def test_delete_raises_when_excerpt_is_public(
    authorized_client, user, bounding_geometry
):
    excerpt = Excerpt.objects.create(
        name="Neverland",
...
def test_delete_raises_when_excerpt_belongs_to_someone_else(
    authorized_client, user, bounding_geometry
):
    another_user = get_user_model().objects.create(
        username=user.username + "2",
        email="{}@example.com".format(user.username + "2"),
        password="test",
    )
    excerpt = Excerpt.objects.create(
        name="Neverland",
...
def test_delete_raises_when_excerpt_has_exports_belonging_to_someone_else(
    authorized_client, user, bounding_geometry
):
    excerpt = Excerpt.objects.create(
        name="Neverland",
