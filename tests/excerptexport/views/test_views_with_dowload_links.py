...
def exports(order, db):
    return [
        order.exports.create(file_format=format_choice[0])
        for format_choice in output_format.CHOICES
    ]
    ...
    return [
        OutputFile.objects.create(
            ...
            public_identifier=UUID(
                int=0x0
            ),  # Nil UUID, see http://tools.ietf.org/html/rfc4122#section-4.1.7
            export=export,
        )
...
def test_send_all_links_mailto_link(
    _, authorized_client, db, downloads, view_with_mailto_links
):
...
def test_send_link_mailto_links(
    _, authorized_client, db, downloads, view_with_mailto_links, expected_html
):
    response = authorized_client.get(view_with_mailto_links, HTTP_HOST="example.com")
    assert response.status_code == 200
    ...
    dummy.assertInHTML(expected_html, actual_response_content)
