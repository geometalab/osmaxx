...
class ExportListTestCase(TestCase, PermissionHelperMixin):
    ...
    def setUp(self):
        ...
        self.extraction_order = ExtractionOrder.objects.create(
            excerpt=excerpt, orderer=user
        )

        self.foreign_extraction_order = ExtractionOrder.objects.create(
            excerpt=excerpt,
            orderer=User.objects.create_user(
                "another_user", "someone_else@example.com", "top secret"
            ),
        )

    def test_redirect_to_login_if_not_logged_in(self, *args):
        response = self.client.get(reverse("excerptexport:export_list"))
        self.assertRedirects(
            response, "/login/?next=/exports/", fetch_redirect_response=False
        )
