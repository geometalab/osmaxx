...
class ExcerptExportViewTests(TestCase, PermissionHelperMixin):
    def setUp(self):
        ...
        multi_polygon = geos.GEOSGeometry(
    ...
    @vcr.use_cassette(
        "fixtures/vcr/views-test_test_new_offers_existing_own_excerpt.yml"
    )
    def test_new_offers_existing_own_excerpt(self):
        ...
        self.assertIn(
            (
                "Personal excerpts (user) [1]",
                ((self.existing_own_excerpt.id, "Some old Excerpt"),),
            ),
            response.context["form"].fields["existing_excerpts"].choices,
        )
        self.assertIn(
            self.existing_own_excerpt.name, response.context["form"].form_html
        )

    @vcr.use_cassette(
        "fixtures/vcr/views-test_test_new_offers_existing_public_foreign_excerpt.yml"
    )
    def test_new_offers_existing_public_foreign_excerpt(self):
        ...
        self.assertIn(
            (
                ...
                (
                    (
                        self.existing_public_foreign_excerpt.id,
                        "Public Excerpt by someone else",
                    ),
                ),
            ),
            response.context["form"].fields["existing_excerpts"].choices,
        ...
        self.assertIn(
            self.existing_public_foreign_excerpt.name,
            response.context["form"].form_html,
        )

    @vcr.use_cassette(
        "fixtures/vcr/views-test_new_doesnt_offer_existing_private_foreign_excerpt.yml"
    )
    ...
    def test_create_when_not_logged_in(self):
        ...
        response = self.client.post(
            reverse("excerptexport:order_new_excerpt"), self.new_excerpt_post_data
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_create_with_new_excerpt(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertEqual(
            Excerpt.objects.filter(
                name="A very interesting region", is_active=True, is_public=False
            ).count(),
            1,
        )
    ...
    def test_create_with_new_excerpt_ignores_ispublic(self):
        ...
        self.assertEqual(
            Excerpt.objects.filter(
                name="A very interesting region", is_public=True
            ).count(),
            0,
        )
    ...
    def test_create_with_existing_excerpt(self):
        ...
        )  # this should be a redirect when successful
        self.assertEqual(
            ExtractionOrder.objects.filter(
                excerpt_id=self.existing_excerpt_post_data["existing_excerpts"]
            ).count(),
            1,
        )  # only reproducible because there is only 1

    @vcr.use_cassette(
        "fixtures/vcr/views-test_create_with_new_excerpt_persists_a_new_order.yml"
    )
    def test_create_with_new_excerpt_persists_a_new_order(self):
        ...
        newly_created_order = (
            ExtractionOrder.objects.first()
        )  # only reproducible because there is only 1
        self.assertEqual(
            newly_created_order.coordinate_reference_system,
            self.coordinate_reference_system,
        )
        self.assertEqual(newly_created_order.detail_level, self.detail_level)
        assert_that(
            newly_created_order.extraction_formats, contains_in_any_order("fgdb")
        )
    ...
    @vcr.use_cassette(
        "fixtures/vcr/views-test_create_with_existing_excerpt_persists_a_new_order.yml"
    )
