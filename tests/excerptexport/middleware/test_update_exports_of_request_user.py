...
def test_update_exports_of_request_user_sets_export_to_failed_if_not_available_on_mediator(
    rf, mocker, export_with_job, user
):
    ...
    with mocker.patch.object(ConversionHelper, "authorized_get"):
        with mocker.patch(
            "osmaxx.job_progress.middleware.update_export_if_stale",
...
def test_update_exports_of_request_user_does_not_change_status_when_exception_occurs(
    rf, mocker, export_with_job, user
):
    def side_effect_function(*args, **kwargs):
        raise Exception()
...
def test_update_exports_of_request_user_sets_export_to_failed_if_status_is_overdue(
    rf, mocker, export, user
):
    ...
    with mocker.patch(
        "osmaxx.excerptexport.models.export.timezone.now",
        side_effect=[now, future, future],
    ):
        export.save()
        with mocker.patch.object(ConversionHelper, "authorized_get"):
            with mocker.patch(
                ...
                from osmaxx.job_progress.middleware import (
                    update_exports_of_request_user,
                )
