...
@pytest.mark.django_db()
def test_conversion_parametrization_creation_success(
    authenticated_api_client, conversion_parametrization_data
):
    response = authenticated_api_client.post(
        reverse("conversion_parametrization-list"),
        conversion_parametrization_data,
        format="json",
    )
...
@pytest.mark.django_db()
def test_conversion_parametrization_creation_fails(
    api_client, conversion_parametrization_data
):
...
@pytest.mark.django_db()
def test_conversion_parametrization_detail_access_success(
    authenticated_api_client, conversion_parametrization, persisted_valid_clipping_area
):
    response = authenticated_api_client.get(
        reverse(
            "conversion_parametrization-detail",
            kwargs={"pk": conversion_parametrization.id},
        )
    )
    assert response.status_code == 200
    data = response.json()
...
@pytest.mark.django_db()
def test_conversion_parametrization_detail_access_fails(
    api_client, conversion_parametrization
):
...
@pytest.mark.django_db()
def test_conversion_job_creation_success(
    authenticated_api_client, conversion_job_data, mocker
):
    start_conversion_mock = mocker.patch(
        "osmaxx.conversion.models.Job.start_conversion"
    )
    response = authenticated_api_client.post(
        reverse("conversion_job-list"), conversion_job_data, format="json"
    )
    data = response.json()
    assert response.status_code == 201
...
@pytest.mark.django_db()
def test_conversion_job_detail_access_success(
    authenticated_api_client, conversion_job, conversion_parametrization
):
    response = authenticated_api_client.get(
        reverse("conversion_job-detail", kwargs={"pk": conversion_job.id})
    )
    assert response.status_code == 200
    data = response.json()
...
@pytest.mark.django_db()
def test_conversion_job_detail_access_fails_with_anonymous_user(
    api_client, conversion_job
):
    response = api_client.get(
        reverse("conversion_job-detail", kwargs={"pk": conversion_job.id})
    )
...
def test_conversion_job_absolute_url_resolves_correct(conversion_job, server_url):
    url = server_url + reverse(
        "conversion_job-detail", kwargs={"pk": conversion_job.id}
    )
    assert conversion_job.get_absolute_url() == url


@pytest.mark.django_db()
def test_conversion_job_creation_enqueues(
    authenticated_api_client, conversion_job_data, rq_mock_return, mocker
):
