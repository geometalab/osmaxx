...
def test_create_garmin_export_calls_(output_zip_file_path, area_name, simple_osmosis_line_string, mocker):
    ...
    Garmin(output_zip_file_path=output_zip_file_path, area_name=area_name, cutted_pbf_file='', polyfile_path='').create_garmin_export()
    assert 2 == subprocess_mock.call_count  # 2 calls from garmin
