...
class GISConverter:
    def __init__(
        ...
        *,
        ...
        self._base_file_name = os.path.basename(base_file_name)
        ...
        self._env = Environment(
            loader=PackageLoader(__package__, os.path.join("symbology", "templates"))
        )
        self._env.globals.update(zip=zip)
        self._detail_level = detail_level
        self._start_time = None

    def create_gis_export(self, geom_in_qgis_display_srs):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self._dump_qgis_symbology(
                data_location, geom_in_qgis_display_srs, target_dir=symbology_dir
            )
    ...
    def _dump_qgis_symbology(self, data_location, geom_in_qgis_display_srs, target_dir):
        ...
        for scale_level in ScaleLevel:
            template = self._env.get_template(
                "OSMaxx_{}.qgs.jinja2".format(scale_level.name.upper())
            )
            ...
            template.stream(
                ...
                os.path.join(
                    qgis_symbology_dir, "OSMaxx_{}.qgs".format(scale_level.name.upper())
                )
            )
        shutil.copytree(
