...
class Parametrization(models.Model):
    out_format = models.CharField(
        verbose_name=_("out format"), choices=output_format.CHOICES, max_length=100
    )
    out_srs = models.IntegerField(
        verbose_name=_("output SRS"),
        help_text=_("EPSG code of the output spatial reference system"),
        null=True,
        blank=True,
        default=4326,
        choices=crs.CHOICES,
    )
    clipping_area = models.ForeignKey(
        ClippingArea, verbose_name=_("Clipping Area"), on_delete=models.CASCADE
    )
    ...
    def __str__(self):
        return _("{}: {} as EPSG:{}").format(
            self.id, self.get_out_format_display(), self.out_srs
        )
...
class Job(models.Model):
    ...
    status = models.CharField(
        _("job status"), choices=status.CHOICES, default=status.RECEIVED, max_length=20
    )
    resulting_file = models.FileField(
        _("resulting file"), upload_to=job_directory_path, null=True, max_length=250
    )
    ...
    queue_name = models.CharField(
        _("queue name"),
        help_text=_("queue name for processing"),
        default="default",
        max_length=50,
        choices=[(key, key) for key in settings.RQ_QUEUE_NAMES],
    )
    ...
    def __str__(self):
        return _("job {} with rq_id {} ({})").format(
            self.id, self.rq_job_id, self.parametrization.clipping_area.name
        )
